...
void FileBrowserActivity::loop() {
  ...
  // Trigger delete confirmation popup after holding Confirm for 1s (Books mode only)
  if (mode == Mode::Books && mappedInput.isPressed(MappedInputManager::Button::Confirm) &&
      mappedInput.getHeldTime() >= GO_HOME_MS && !files.empty() && !deleteLongPressTriggered) {
  ...
  if (mappedInput.wasReleased(MappedInputManager::Button::Confirm)) {
    ...
    if (mode == Mode::PickFirmware && !isDirectory) {
      ActivityResult res{FilePathResult{basepath + entry}};
