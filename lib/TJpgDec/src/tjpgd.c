...
static JD_FASTPATH void block_idct(int32_t* src, /* Input block data (de-quantized and pre-scaled for Arai Algorithm) */
...
static JD_FASTPATH JRESULT mcu_load(JDEC* jd /* Pointer to the decompressor object */
) {
  ...
  for (blk = 0; blk < nby + 2; blk++) {    /* Get nby Y blocks and two C blocks */
    ...
    if (cmp && jd->ncomp != 3) { /* Clear C blocks if not exist (monochrome image) */
      ...
      if (JD_FORMAT != 2 || !cmp) {                       /* C components may not be processed if in grayscale output */
        if (z == 1 || (JD_USE_SCALE && jd->scale == 3)) { /* If no AC element or scale ratio is 1/8, IDCT can be ommited
                                                             and the block is filled with DC value */
...
static JRESULT mcu_output(JDEC* jd,                             /* Pointer to the decompressor object */
  ...
  rx = (x + mx <= jd->width)
           ? mx
           : jd->width - x; /* Output rectangular size (it may be clipped at right/bottom end of image) */
  ...
  if (!JD_USE_SCALE || jd->scale != 3) { /* Not for 1/8 scaling */
    ...
    if (JD_FORMAT != 2) { /* RGB output (build an RGB MCU from Y/C component) */
      ...
      /* CrossPoint fix for an upstream TJpgDec bug: with JD_FASTDECODE>=1 the
      /  IDCT (block_idct) and the DC-only/1/8 fill in mcu_load write unclamped
      /  int16_t into mcubuf. The colour path runs them through BYTECLIP during
      /  YCbCr->RGB, but the grayscale path stored the Y value raw, so IDCT
      /  overshoot near sharp black/white edges (e.g. cover text) wrapped around
      /  modulo 256 -- producing black speckle in white areas. Clamp on output. */
      for (iy = 0; iy < my; iy++) {
        py = jd->mcubuf + iy * 8;
        if (my == 16) { /* Double block height? */
          if (iy >= 8) py += 64;
        ...
        for (ix = 0; ix < mx; ix++) {
          ...
          *pix++ = BYTECLIP(*py++); /* Get and store a Y value as grayscale (CrossPoint: clamp; see note below) */
        }
      }
    }
    ...
    for (iy = 0; iy < my; iy += 8) {
      ...
      for (ix = 0; ix < mx; ix += 8) {
        ...
        if (JD_FORMAT != 2) {
          ...
          *pix++ = BYTECLIP(yy); /* CrossPoint: clamp; see note below */
        }
      }
    }
  }
...
JRESULT jd_prepare(JDEC* jd,                                  /* Blank decompressor object */
  ...
  memset(jd, 0, sizeof(JDEC)); /* Clear decompression object (this might be a problem if machine's null pointer is not
                                  all bits zero) */
