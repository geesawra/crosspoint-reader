...
size_t InflateReader::ringSizeFor(const size_t expectedOutputSize) {
  ...
  return (expectedOutputSize == 0) ? INFLATE_DICT_SIZE
...
bool InflateReader::init(const bool streaming, const size_t expectedOutputSize) {
  ...
  size_t dictSize = 0;
  if (streaming) {
    dictSize = ringSizeFor(expectedOutputSize);
...
bool InflateReader::initWithExternalRing(uint8_t* ring, const size_t ringSize) {
  deinit();
  if (!ring || ringSize == 0) return false;
  ringBuffer = ring;
  ownsRing_ = false;
  memset(ringBuffer, 0, ringSize);
  uzlib_uncompress_init(&decomp, ringBuffer, ringSize);
  return true;
}

void InflateReader::deinit() {
  if (ringBuffer) {
    if (ownsRing_) free(ringBuffer);
    ringBuffer = nullptr;
  }
  ownsRing_ = true;
